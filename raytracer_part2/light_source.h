...
class LightSource {
public:
	virtual void shade( Ray3D&, double shadowFactor) = 0;
...
public:
	PointLight( Point3D pos, Colour col, int lid) : 
	_pos(pos), 
	_col_ambient(col), 
	_col_diffuse(col), 
	_col_specular(col) {
	...
	PointLight( Point3D pos, Colour ambient, Colour diffuse, Colour specular, int lid) : 
	_pos(pos), 
	...
	void shade( Ray3D& ray, double shadowFactor );
...
private:
	int id; // Determine if main or not 
	Point3D _pos;
	Colour _col_ambient;
	Colour _col_diffuse; 
